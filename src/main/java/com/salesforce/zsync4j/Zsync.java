...
public class Zsync {
  ...
  public static class Options {
    ...
    /**
     * Corresponds to zsync -i parameter: location of an input file from which to directly copy
     * matching blocks to the output file to reduce the number of bytes that have to be fetched from
     * the remote source.
     *
     * @param inputFile
     * @return
    ...
    /**
     * Input files to construct output file from. If empty and the output file does not yet exist,
     * the full content is retrieved from the remote location.
     *
     * @return
     */
     ...
     * relative path. If the output file already exists, it will also be used as an input file to
     * reuse matching blocks. Upon completion of the zsync operation, the output file is atomically
     * replaced with the new content (with fall-back to non-atomic in case the file system does not
     * support it).
     *
     * @return
     */
  ...
  /**
   * Convenience method for {@link #zsync(URI, Options, ZsyncObserver)} without options or observer.
   * The URI passed to this method must be an absolute HTTP URL. The output file location will be
   * derived from the filename header in the zsync control file as described in
   * {@link Options#getOutputFile(Path)}. If the output file already exists, zsync will reuse
   * unchanged blocks, download only changed blocks, and replace the output file upon successful
   * completion. Otherwise, zsync will download the full content from the remote server.
   *
  ...
  public Path zsync(URI zsyncFile) throws ZsyncException {
    return zsync(zsyncFile, null);
  }

  /**
   * Convenience method for {@link #zsync(URI, Options)} without observer.
   *
   * @param zsyncFile URI of the zsync control file generated for the target file by
   *        {@link ZsyncMake}
   ...
   *
   * <p>
   * The zsyncFile parameter must point to the zsync control file generated by {@link ZsyncMake}.
   * Typically, this will be an absolute HTTP URI, but it can also be an absolute or relative file
   * system URI in case the file has already been downloaded. The URI of the actual file to download
   * is determined from the <code>URI</code> header value in the zsync control file. If that value
   * is a relative URI, it is resolved against the remote zsync file URI. For example, if the
   * control file is located at <code>http://myhost.com/file.zsync</code> and the header value is
   * <code>file</code>, then the resolved URI is <code>http://myhost.com/file</code>. Note that if
   * the zsyncFile parameter is a local file system URI and the URI header value is a relative URI,
   * the {@link Options#getZsyncFileSource()} option must be set to resolve the absolute URI.
   * </p>
   * <p>
   * The options parameter is optional, i.e. it may be null or empty. It can be used to pass
   * optional parameters to zsync per the documentation on the get and set methods on the
   * {@link Options} class. For example, additional input files can be specified via
   * {@link Options#addInputFile(Path)}; the output location can be set via
   * {@link Options#setOutputFile(Path)}.
   * </p>
   * <p>
   * The observer parameter is also optional. Passing a {@link ZsyncObserver} observer enables
   * fine-grained progress and statistics reporting. For the latter {@link ZsyncStatsObserver} can
   * be used directly.
   * </p>
   * 
   * @param zsyncFile URI of the zsync control file generated for the target file by
